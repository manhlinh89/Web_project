...
@NamedNativeQuery(
        ...
        query = "select p.name as label, sum(o.product_id) as value from product p " +
                "inner join orders o on p.id = o.product_id " +
                "where o.status = 3 and date_format(o.created_at,'%m') = ?1 " +
                "and date_format(o.created_at,'%Y') = ?2 " +
                "group by p.id order by sum(o.product_id) desc "
)

@NamedNativeQuery(
        name = "getProductOrders_test",
        resultSetMapping = "chartProductDTO",
        query = "select p.name as label, count(o.product_id) as value from product p \n" +
                "                inner join orders o on p.id = o.product_id \n" +
                "                where o.status =3\n" +
                "                group by p.name \n" +
